...
export const rankRoute = router.post("/admin/addRank", async (req, res) => {
  ...
  if(NameRank && minScoreRank && maxScoreRank && rewardRank) {
    if(typeof minScoreRank != 'number'  || typeof maxScoreRank != 'number' || typeof rewardRank != 'number') {
    ...
    if(minScoreRank >= maxScoreRank) {
       return  res.sendError(403, "minScore can not be more than maxScore")
        ...
        for (const itRank of ranks) {
          if(
            minScoreRank >= itRank.minScore && maxScoreRank <= itRank.maxScore ||
            maxScoreRank >= itRank.minScore && maxScoreRank <= itRank.maxScore || 
            itRank.minScore >= minScoreRank && itRank.maxScore <= maxScoreRank
        ){
            return res.sendError(403, "Logical Problem \n overlaping data records");
        }
        }
        
    }

    const itNewRank = {name: NameRank, minScore: minScoreRank, maxScore: maxScoreRank, reward: rewardRank};
