...
export class createWallet {
    ...
    public async createNewWallet(userId:string) {
        ...
        let wallet = ton.WalletContractV4.create({publicKey: keyPair.publicKey, workchain: 0});
        return {
    ...
    public async getTransactionUserIncome(lastHashTL:string,mne:string[],pass:string){
        ...
        transaction.forEach(itData=>{
            if (itData.inMessage?.info.value!==undefined && itData.inMessage.body!==undefined){
                const hexString = itData.inMessage.body.toString()
                const hexWithoutPrefix = hexString.substring(3, hexString.length - 1).replace(/^0+/, "");
    ...
    public async getTransactionUserOutcome(lastHashTL:string,mne:string[],pass:string){
        ...
        transaction.forEach(itData=>{
            if (itData.outMessages.values()!==undefined && itData.outMessages.length!=0 && itData.outMessages.get(0).info.value.coins.valueOf()>0){
...
function generateStrongPassword(length,str) {
    let password = '';
